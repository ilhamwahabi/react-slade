...
class Slade extends Component {
  ...
  previousSlade = (previousSlade, index) => {
    if (!this.isStartItem(index)) previousSlade();
  };

  nextSlade = (nextSlade, index, items) => {
    if (!this.isEndItem(index, items)) nextSlade();
  };
  ...
  renderArrow = (rightArrow, index, items, start) =>
    typeof rightArrow === 'string' ? (
      <Image isEdge={this.isEdge(index, items, start)} start={start} src={rightArrow} alt="arrow" />
    ) : (
      <Svg isEdge={this.isEdge(index, items, start)} start={start} width="100" height="60">
        {rightArrow}
      </Svg>
    );

  render() {
    const { open, items, index, backdrop, rightArrow, closeSlade, previousSlade, nextSlade } = this.props;
      ...
      <Fragment>
        ...
        <StyledSlade onClick={closeSlade} open={open}>
          <Arrow
            ...
            onClick={e => {
              e.stopPropagation();
              this.previousSlade(previousSlade, index);
            }}
          >
            {this.renderArrow(rightArrow, index, items, 1)}
          </Arrow>
          <Dialog onClick={e => e.stopPropagation()}>{items[index]}</Dialog>
          <Arrow
            end={1}
            onClick={e => {
              e.stopPropagation();
              this.nextSlade(nextSlade, index, items);
            }}
          >
